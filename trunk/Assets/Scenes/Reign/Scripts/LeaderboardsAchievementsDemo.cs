...
public class LeaderboardsAchievementsDemo : MonoBehaviour
{
	...
	void OnGUI()
	{
		...
		{
			GUI.Label(new Rect(0, 0, Screen.width, Screen.height/8), "Authenticated UserID: " + ScoreManager.UserID);
			...
			// Report Scores
			if (GUI.Button(new Rect(Screen.width-256, 0, 256, 64), "Report Random Score") || Input.GetKeyUp(KeyCode.S))
			{
				ScoreManager.ReportScore("Level1", Random.Range(0, 500), reportScoreCallback);
			}

			// Report Achievements
			if (GUI.Button(new Rect(Screen.width-256, 64, 256, 64), "Report Random Achievement") || Input.GetKeyUp(KeyCode.R))
			{
			...
			if (GUI.Button(new Rect((Screen.width/2)-(256/2), (Screen.height/2)-(64/2), 256, 64), "Logout") || Input.GetKeyUp(KeyCode.O))
			{
				ScoreManager.Logout();
			}
		}
